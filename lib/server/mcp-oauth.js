...
export function registerOAuthClient(rawMetadata) {
  ...
  if (!redirectUris.every(isAllowedRedirectUri)) {
    throw new Error('All redirect_uris must use HTTPS or localhost HTTP');
  }
  ...
  const tokenEndpointAuthMethod = metadata.token_endpoint_auth_method || 'client_secret_post';
